...
Each case directory contains an `alert.json` whose `_meta.scoring_points.expected_root_cause` and `_meta.scoring_points.miss_notes` carry the rubric for grading. Commit the directory under `tests/benchmarks/` and the next benchmark run will include the new regressions.
