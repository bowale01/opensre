...
class JsonlSessionRepo:
    ...
    @staticmethod
    def _summary(path: Path, header: dict[str, Any], entries: list[dict[str, Any]]) -> dict[str, Any]:
...
def _load_v2_lines(lines: list[str]) -> tuple[dict[str, Any], list[dict[str, Any]]] | None:
    ...
    try:
        ...
        return None
    if not isinstance(header, dict) or header.get("type") != "session" or header.get("version") != 2:
        return None
...
def _resolve_entry_id(entries: list[dict[str, Any]], entry_ref: str | None) -> str | None:
    if entry_ref:
        matches = [str(rec.get("id")) for rec in entries if str(rec.get("id", "")).startswith(entry_ref)]
...
def _turn_details_for_branch(branch: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for rec in branch:
        ...
        role = rec.get("role")
        if role == "user":
            pending_user = {"kind": metadata.get("kind", "chat"), "prompt": rec.get("content") or ""}
